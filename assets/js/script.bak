...
function translate() {
    ...
    document.querySelector('.goog-logo-link')?.setAttribute('rel', 'noopener');
    const googleCombo = document.querySelector("select.goog-te-combo");
    const langSelect = document.querySelector('.dropdown-lang');
    const dropdownContainer = document.querySelector('.dropdown');
    const dropbtn = document.querySelector('.dropbtn'); // Define dropbtn here
    ...
    langSelect?.addEventListener('click', function (event) {
        const { target } = event;
        if (!target) return;
        document.querySelector('.lang-select.selected')?.classList.remove('selected');
    ...
    const checkSelectedLangInterval = setInterval(function () {
        ...
        if (selectedLang) {
            document.querySelector('.lang-select.selected')?.classList.remove('selected');
            const initialLang = document.querySelector(`.lang-select[data-lang="${selectedLang}"]`);
            if (initialLang) {
                document.querySelector('.lang-select.selected')?.classList.remove('selected');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('main')?.querySelectorAll('p, h1, h2, h3, h4, h5, h6')?.forEach(wrapElement);
});